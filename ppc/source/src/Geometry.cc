...
/*
   ...
   G4Material* Herium3 = new G4Material("Herium3",density=1.346*mg/cm3,ncomponents=1,kStateGas,temperature=300.15*kelvin,pressure=10.*atmosphere);
   Herium3->AddElement(He3,1);
   /*
   G4Material* materi_Rock =
     new G4Material("materi_Rock",density = 3.0*g/cm3,ncomponents=10);
   materi_Rock->AddMaterial(SiO2,fractionmass=0.3560);
   materi_Rock->AddMaterial(Al2O3,fractionmass=0.1130);
   materi_Rock->AddMaterial(Fe2O3,fractionmass=0.1090);
   materi_Rock->AddMaterial(MnO,fractionmass=0.0108);
   materi_Rock->AddMaterial(MgO,fractionmass=0.0099);
   materi_Rock->AddMaterial(CaO,fractionmass=0.3963);
...
// Define 'lab'
   ...
   G4int copyNum_LogV = 2000;                // Set ID number of LogV
...
// Define 'PPC Detector'
   ...
   // Define logical volume
   G4Material* materi_PPC = materi_Man->FindOrBuildMaterial("G4_AIR");
...
// Sensitive volume
    ...
    logVol_PPC->SetSensitiveDetector(aSV);         // Add sensitivity to the logical volume
